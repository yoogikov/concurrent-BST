type 'a snapshot = { flag : bool; tag : bool; value : 'a }
...
(** [get r] atomically reads the current [flag], [tag], and [value] of [r] and
    returns them as a {!snapshot}. *)
let get r =
  (* not_implemented "get" *)
  Atomic.get r

(** [set r flag tag value] replaces the contents of [r] with the given [flag]
    [tag] and payload [value]*)
let set r ~flag ~tag value =
  (* not_implemented "set" *)
  Atomic.set r { flag; tag; value }

(** Return [true] on a successful cas and [false] otherwise*)
let cas (snapshot : 'a t) ~exp_flag ~exp_tag ~(exp_val : 'a) ~new_flag ~new_tag
    ~(new_val : 'a) =
  let rec loop () =
    let current = Atomic.get snapshot in
    if
      current.flag <> exp_flag || current.tag <> exp_tag
